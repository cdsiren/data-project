...
describe("R2 Cold Storage Integration", () => {
  /**
   * R2 tests require running via Wrangler to get access to the R2 binding.
   * Run with: wrangler dev --test
   *
   * These tests are designed to run in a Cloudflare Workers environment.
   * When running with vitest directly, they will be skipped.
   */

  // Check if we're in a Workers environment with R2 access
  const hasR2Access = typeof globalThis !== "undefined" && "ORDERBOOK_STORAGE" in globalThis;

  it.skipIf(!hasR2Access)("should write snapshot to R2", async () => {
    // This test only runs in Workers environment
    const r2 = (globalThis as unknown as { ORDERBOOK_STORAGE: R2Bucket }).ORDERBOOK_STORAGE;

    const testData = {
      asset_id: "r2_test_" + Date.now(),
      timestamp: Date.now(),
      bids: [{ price: 0.5, size: 100 }],
      asks: [{ price: 0.6, size: 100 }],
    };

    const key = `test/snapshots/${testData.asset_id}.json`;
    await r2.put(key, JSON.stringify(testData), {
  ...
  it("should verify R2 write format matches expected schema", () => {
    // This test verifies the format we'll use for R2 archival
    const date = new Date();
    ...
    const expectedKeyFormat = `snapshots/${dateStr}/${snapshot.asset_id}/${hour}.jsonl`;
    expect(expectedKeyFormat).toMatch(/^snapshots\/\d{4}-\d{2}-\d{2}\/test_asset\/\d{2}\.jsonl$/);

    // Verify snapshot serializes correctly
    const serialized = JSON.stringify(snapshot);
    const parsed = JSON.parse(serialized);
    expect(parsed.asset_id).toBe(snapshot.asset_id);
